...
public class Interpreter : IInterpreter, IVisitor<object?>
{
    ...
    private Dictionary<(int x, int y), SKColor> _currentDraw = new();
    ...
    public object? VisitDrawLineStmt(DrawLineStmt stmt)
    {
        ...
        for (var i = 0; i < distance; i++)
        {
            ...
            DrawBrush(currentX, currentY, color);
        }
    ...
    public object? VisitDrawRectangleStmt(DrawRectangleStmt stmt)
    {
        ...
        if (_isFilling)
            FilledRectangle(startX, startY, width, height);
        else
    ...
    {
        if (!IsInBounds((int)_currentWallEPosition.X, (int)_currentWallEPosition.Y)) return null;
        ...
        fillSkColor = BlendColors(targetSkColor, fillSkColor);
    ...
    public object VisitCallExpr(CallExpr expr)
    {
        ...
        if (expr.CalledFunction == "GetColorCount")
        {
            ...
            var x1 = ConvertToInt(evaluatedArgs[1], expr.Arguments[1].Location, "x for GetColorCount");
            var y1 = ConvertToInt(evaluatedArgs[2], expr.Arguments[2].Location, "y for GetColorCount");
            var x2 = ConvertToInt(evaluatedArgs[3], expr.Arguments[3].Location, "width for GetColorCount");
            var y2 = ConvertToInt(evaluatedArgs[4], expr.Arguments[4].Location, "height for GetColorCount");
            ...
            //ToDo: Arreglar
            //Retorna la cantidad de casillas con color color que hay en el rect´angulo formado por las posiciones
            //x1,y1 (una esquina) y x2, y2(la otra esquina). Si cualquiera de las esquinas cae fuera de las dimensiones del
            //canvas, debe retornar 0.
            if (x2 <= 0 || y2 <= 0) return new IntegerOrBool(0);
            var count = 0;
            var startX = Math.Max(0, x1);
            var startY = Math.Max(0, y1);
            var endX = Math.Min(_skBitmap.Width, x1 + x2);
            var endY = Math.Min(_skBitmap.Height, y1 + y2);
    ...
    private void DrawPixel(int x, int y, SKColor color)
    {
        if (!IsInBounds(x, y)) return;
        _skBitmap.SetPixel(x, y, color);
    }

    private void DrawBrush(int cx, int cy, SKColor color)
    {
        ...
        for (var y = startY; y < endY; y++)
        {
            ...
            if (_currentDraw.ContainsKey((x, y))) continue;
            _currentDraw.Add((x, y), skColor);
        }
    }
    ...
    private void DrawCircle(int centerX, int centerY, int radius)
    {
        ...
        while (y >= x)
        {
            DrawBrush(centerX + x, centerY + y, color);
            DrawBrush(centerX - x, centerY + y, color);
            DrawBrush(centerX + x, centerY - y, color);
            DrawBrush(centerX - x, centerY - y, color);
            DrawBrush(centerX + y, centerY + x, color);
            DrawBrush(centerX - y, centerY + x, color);
            DrawBrush(centerX + y, centerY - x, color);
            DrawBrush(centerX - y, centerY - x, color);
    ...
    private void DrawRectangle(int x, int y, int width, int height)
    {
        ...
        for (var i = 0; i < width; i++) DrawBrush(x + i, y, color);
        for (var i = 0; i < width; i++) DrawBrush(x + i, y + height - 1, color);
        for (var i = 0; i < height; i++) DrawBrush(x, y + i, color);
        for (var i = 0; i < height; i++) DrawBrush(x + width - 1, y + i, color);


        DrawCurrent();
    }

    private void FilledRectangle(int x, int y, int width, int height)
    {
        var color = ToSkiaColor(_currentColor);

        for (var j = 0; j < height; j++)
        for (var i = 0; i < width; i++)
            DrawBrush(x + i, y + j, color);
    ...
    private void DrawHorizontalLine(int x0, int x1, int y, SKColor color)
    {
        ...
        for (var x = x0; x <= x1; x++) DrawBrush(x, y, color);
    }
}